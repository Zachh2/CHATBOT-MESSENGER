Lưu Ý! Đây Là Sản Phẩm Được Phúc Remake ( Chính Bởi Facebook-Chat-Api Của Schmavery, Tác Giả Không Chịu Trách Nghiệm Nào !), Nếu Có Lỗi Hãy Thử Sử Dụng Sang Sản Phẩm Khác !
...
### Tải Bản Mới Nhất Hoặc Update
    ...
    ```bash
    npm install fb-xuyen-get@latest
```
Hoặc
```bash
npm i fb-xuyen-get@latest
```
...
## Cách Sử Dụng
```javascript
const login = require("fb-xuyen-get"); // lấy từ lib ra 
...
```js
const login = require("fb-xuyen-get");
