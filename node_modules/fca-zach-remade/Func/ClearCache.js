...
module.exports = function (defaultFuncs, api, ctx) {
    return function (Args,callback) {
        ...
        if (!Args.New || utils.getType(Args.New) !== "Array") { 
            New1 = Recommend;
            log.Normal("No further adding, proceed to use according to the system's instructions!");
        }
        else {
            for (let i = 0; i < Args.New.length; i++) {
                if (Object.indexOf(Args.New[i]) === -1) {
                    log.Normal('File not found ' + Args.New[i] + ' in the list of formats');
                    return;
                }
        ...
        switch (process.platform) {
            case 'linux': {
                ...
                log.Normal('Successfully cleared ' + New1.length + ' File type!');
