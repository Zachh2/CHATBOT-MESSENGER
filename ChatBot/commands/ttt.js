...
module.exports.run = async function ({ event, api, args }) {
  ...
  let newData;
  if (args.length == 0) return api.sendMessage(`Please select x or o\nExample: ${global.config.PREFIX}ttt x`, threadID, messageID);
  ...
  if (!data.gameOn) {
    ...
    if (abc == "o") {
      ...
      api.sendMessage({ body: "You go first!\nReply the 1-9 number of cells to check", attachment: await displayBoard(newData)}, threadID, (error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
    ...
    if (abc == "x") {
      ...
      AIStart(newData);
      api.sendMessage(`Please wait The Ai is picking`, threadID, messageID);
      api.sendMessage({ body: "I go first!\nReply 1-9 number of cells to check", attachment: await displayBoard(data)}, threadID,(error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
